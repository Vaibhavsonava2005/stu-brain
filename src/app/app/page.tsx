                    ...
                    <tr key={i} style={{borderTop:'1px solid rgba(108,99,255,.12)'}}>
                      ...
                      <td style={{padding:'9px 12px'}}>
                        <div style={{display:'flex',gap:5,flexWrap:'wrap'}}>
                          ...
                          <button onClick={()=>{if(confirm('⚠️ PERMANENTLY DELETE '+sc.name+'?\n\nThis will delete:\n• All students and their progress\n• All teachers\n• All certificates\n• All challenges\n\nThis CANNOT be undone!')){fetch('/api/superadmin/schools',{method:'DELETE',headers:{'Content-Type':'application/json',authorization:`Bearer ${token}`},body:JSON.stringify({id:sc.id})}).then(r=>r.json()).then(d=>{if(d.success){alert('✅ School deleted permanently.');token&&loadSuperAdmin(token);}else alert('❌ '+(d.error||'Error'));});}}} style={{background:'rgba(255,50,50,.2)',color:'#FF4444',border:'1px solid rgba(255,50,50,.4)',borderRadius:6,padding:'3px 8px',fontSize:10,fontWeight:800,cursor:'pointer'}}>🗑️ Delete</button>
                        </div>
                      </td>
                    </tr>
                  ))}
